	...
	if (-1 == ret)
	  {
	    m_SockConn->close();
	    m_SockConn = NULL;
	    //OsSysLog::add(FAC_SIP, PRI_ERR, "%d -----------------> %d", len, ret);
	  }
      }
  }
};
  ...
  if (!m_Server->isOk())
    {
      ...
      //OsSysLog::add( FAC_SIP, PRI_ERR, "MySockServ: port not ok" );
    }
   ...
   case OsMsg::PHONE_APP:
   {
	 ...
	 sipMsg->getContactEntry(0, &contact);
               ...
               else
		 {
	      ...
	      {
		OsSysLog::add(FAC_SIP, PRI_ERR, "SipXProxyCstaObserver - no Cseq in response");
	      }
         }

#        ifdef LOG_DEBUG
         OsSysLog::add(FAC_SIP, PRI_INFO, "SipXProxyCstaObserver message is %s",
...
#        endif
      ...
      {
         OsSysLog::add(FAC_SIP, PRI_INFO, "SipXProxyCstaObserver getMessage returned NULL");
      }
   }
   break;
   ...
   default:
   {
      OsSysLog::add(FAC_SIP, PRI_INFO, "SipXProxyCstaObserver invalid message type %d", msgType );
   }
   } // end switch (msgType)
