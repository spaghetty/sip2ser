...
%package 	ruby
Summary:	Ruby support for the FreeSWITCH open source telephony platform
Group:		System/Libraries
Requires:	%{name} = %{version}-%{release}
Requires:       ruby
...
#%{__mkdir} -p %{buildroot}%{ruby_sitelib}
%{__install} -D -m 644 ruby/ESL.so %{buildroot}%{ruby_sitelib}/ESL.so
cd ../..
...
%files ruby
%defattr(-,freeswitch,daemon)
%attr(0755, root, bin) %{ruby_sitelib}/ESL.so*
